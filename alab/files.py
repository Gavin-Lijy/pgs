...
class modelgroup(object):
    def __init__(self,grouphandler,genome,idx):
        try:
            ...
            raise ex
        self.score  = float(re.findall(r'Final score (\d+.\d+)',self.log)[0])
        self.genome = genome
        self.idx    = idx

        findRes = re.findall(r'#of Intra restraints: (\d+) #of Inter restraints: (\d+)',self.log)
        self.intraRestraints = float(findRes[0][0])
        self.interRestraints = float(findRes[0][1])
        findVio = re.findall(r'(\d+) violations in total',self.log)
